...
try:
    ...
    def tqdm(x, **kwargs):
        # Fallback: if tqdm isn't installed, return the iterable unchanged
        return x
...
VALID_LABEL_TYPES = ["boolean", "numerical", "survival", "categorical", "multilabel"]
...
class Label:
    ...
    value: Union[bool, int, float, SurvivalValue, List[int], List[str]]
...
def load_labeled_patients(filename: str) -> "LabeledPatients":
    """Load labels from a CSV produced by `LabeledPatients.save()`.

    Special handling for `multilabel`:
      - Accept values serialized as JSON (preferred) or Python-literal lists (fallback).
      - Normalize to a Python `list` in-memory for downstream use.
    """
    ...
    for row in rows:
        ...
        if labeler_type == "survival":
            value: Union[bool, SurvivalValue, int, float, List[int], List[str]] = SurvivalValue(
                time_to_event=datetime.timedelta(minutes=float(v)),
                is_censored=str(row.get("is_censored", "")).lower() == "true",
            )
        elif labeler_type == "boolean":
            value = str(v).strip().lower() == "true"
        elif labeler_type == "categorical":
            value = int(v)
        elif labeler_type == "numerical":
            value = float(v)
        elif labeler_type == "multilabel":
            vv = (v or "").strip()
            if not vv:
                parsed_list: Any = []
            else:
                # Prefer JSON (what `save()` writes); fallback to safe Python literal parsing.
                try:
                    parsed_list = json.loads(vv)
                except Exception:
                    try:
                        parsed_list = ast.literal_eval(vv)
            ...
            raise ValueError(f"Unsupported label_type: {labeler_type}")

        t = datetime.datetime.fromisoformat(row["prediction_time"])
        t = t.replace(second=0, microsecond=0, tzinfo=None)
        labels[int(row["patient_id"])].append(Label(time=t, value=value))
...
class LabeledPatients(MutableMapping[int, List[Label]]):
    ...
    def __init__(
        ...
        Args:
            patients_to_labels: [key] = patient ID, [value] = labels for this patient
            labeler_type: Type of labeler
        """
    ...
    def save(self, target_filename) -> None:
        with open(target_filename, "w") as f:
            ...
            for pid in pids:
                # Sort by time only; list-valued labels are not directly comparable.
                labels = sorted(self.patients_to_labels[pid], key=lambda x: x.time)
                ...
                for label in labels:
                    ...
                    elif self.labeler_type == "multilabel":
                        writer.writerow(
                            [
                                pid,
                                label.time.isoformat(),
                                self.labeler_type,
                                json.dumps(label.value, ensure_ascii=False),
                            ]
                        )
                    else:
                        writer.writerow([pid, label.time.isoformat(), self.labeler_type, label.value])
    ...
    def as_numpy_arrays(self) -> Tuple[NDArray, NDArray, NDArray]:
        ...
        # Multi-label values are lists, so use object dtype.
        return (
            np.array(patient_ids),
            np.array(label_values, dtype=object),
    ...
    def get_patients_with_label_values(self, values: List[Any]) -> List[int]:
        """Return IDs of patients with at least one label whose value is in `values`.

        For multilabel: include a patient if ANY element in the label list is in `values`.
        """
        ...
        for patient, labels in self.items():
            for label in labels:
                if self.labeler_type == "multilabel":
                    if isinstance(label.value, list) and any(v in target_set for v in label.value):
                        patient_ids.add(patient)
                        break
                else:
                    # Avoid implicit type coercions (e.g., 1.0 == True).
                    for v in values:
    ...
    def load_from_numpy(
        ...
        for patient_id, l_value, l_time in zip(patient_ids, label_values, label_times):
            if labeler_type in ["boolean", "numerical", "categorical"]:
                patients_to_labels[int(patient_id)].append(Label(time=l_time, value=l_value))
            elif labeler_type == "multilabel":
                val = l_value.tolist() if hasattr(l_value, "tolist") else l_value
                if not isinstance(val, list):
                    raise ValueError("multilabel expects list-like values")
                patients_to_labels[int(patient_id)].append(Label(time=l_time, value=val))
            elif labeler_type == "survival":
                patients_to_labels[int(patient_id)].append(
                    Label(
                        time=l_time,
...
class Labeler(ABC):
    ...
    def label(self, patient: Patient) -> List[Label]:
        ...
        Returns:
            A list of `Label` containing every label for the given patient
        """
        pass
    ...
    def apply(
        ...
        # Workaround to pickling limitations: drop ontology pre-fork, restore in subprocess.
        ...
        chunksize = max(1, total // max(1, num_threads * 8))

        if num_threads == 1:
            iterator = tqdm(map(_apply_labeling_function, tasks), total=total, desc="Labeling (1 proc)", mininterval=0.5)
            results = [res for res in iterator]
        else:
            ctx = multiprocessing.get_context("forkserver")
            with ctx.Pool(num_threads) as pool:
                iterator = pool.imap_unordered(_apply_labeling_function, tasks, chunksize)
                iterator = tqdm(iterator, total=total, desc=f"Labeling ({num_threads} procs)", mininterval=0.5)
                results = [res for res in iterator]
